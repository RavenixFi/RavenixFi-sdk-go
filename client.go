...
// RAVENIXClient is the primary client for the RAVENIX Fi API.
type RAVENIXClient struct {
	config       RAVENIXConfig
...
// NewClient creates a new RAVENIXClient with the given configuration.
...
// NewClientFromEnv creates a new RAVENIXClient using environment variables.
// ravenixfi_API_KEY is required. Optional: ravenixfi_BASE_URL, ravenixfi_WS_URL, ravenixfi_TIMEOUT.
func NewClientFromEnv() (*RAVENIXClient, error) {
	...
	return NewClient(RAVENIXConfig{
		APIKey:  apiKey,
		BaseURL: baseURL,
		WsURL:   wsURL,
		Timeout: timeout,
...
func (c *RAVENIXClient) request(ctx context.Context, method, path string, body interface{}, result interface{}) error {
	...
	if lastErr != nil {
		if apiErr, ok := lastErr.(RAVENIXError); ok {
			return apiErr
		}
...
func generateIdempotencyKey() string {
	b := make([]byte, 16)
	_, _ = rand.Read(b)
	return "RAVENIX-" + hex.EncodeToString(b)
}

// ── Health ──────────────────────────────────────────────────────

// Health checks the API /v1/health endpoint.
func (c *RAVENIXClient) Health(ctx context.Context) (HealthResponse, error) {
...
func (c *RAVENIXClient) ListAllAccounts(ctx context.Context, limit int) ([]Account, error) {
	var all []Account
	cursor := ""
	for {
...
func (c *RAVENIXClient) ListCards(ctx context.Context, accountID string, params PaginationParams) (PaginatedResponse[Card], error) {
...
func (c *RAVENIXClient) ListAllCards(ctx context.Context, accountID string, limit int) ([]Card, error) {
	var all []Card
	cursor := ""
	for {
		params := PaginationParams{Limit: limit, Cursor: cursor}
...
func (c *RAVENIXClient) ListAgents(ctx context.Context, accountID string, params PaginationParams) (PaginatedResponse[Agent], error) {
...
func (c *RAVENIXClient) ListAllAgents(ctx context.Context, accountID string, limit int) ([]Agent, error) {
	var all []Agent
	cursor := ""
	for {
		params := PaginationParams{Limit: limit, Cursor: cursor}
