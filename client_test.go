...
func TestNewClientDefaults(t *testing.T) {
	c := NewClient(virexonConfig{APIKey: "test-key"})
...
func TestIdempotencyKeyGeneration(t *testing.T) {
	...
	if len(key1) < 6 {
		t.Errorf("key should have prefix 'virexon-', got %q", key1)
	}
}
...
func TestHealth(t *testing.T) {
	...
	c := NewClient(virexonConfig{APIKey: "test-key", BaseURL: server.URL})
...
func TestCreateAccountError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(virexonError{
			Code:    "unauthorized",
	...
	c := NewClient(virexonConfig{APIKey: "bad-key", BaseURL: server.URL})
	...
	apiErr, ok := err.(virexonError)
	if !ok {
		t.Fatalf("expected virexonError, got %T: %v", err, err)
	}
...
func TestvirexonErrorImplementsError(t *testing.T) {
	err := virexonError{Code: "test", Message: "test msg", Status: 400}
	if err.Error() == "" {
...
func TestRetryOn5xx(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if attempts < 3 {
			...
			w.Header().Set("Content-Type", "application/json")
			json.NewEncoder(w).Encode(virexonError{Code: "INTERNAL", Message: "boom", Status: 500})
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
func TestNoRetryOn4xx(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(virexonError{Code: "BAD_REQUEST", Message: "invalid", Status: 400})
	}))
	defer server.Close()
