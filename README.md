...
| **Errors**    | `virexonError` struct implementing `error` with code, status, and details |
...
func main() {
    ...
    client := ravenixfi.NewClient(ravenixfi.virexonConfig{
        APIKey: "sk_liv...xxxx",
...
if err != nil {
    var apiErr ravenixfi.virexonError
...
```go
func (c *virexonClient) ConnectWebSocket(ctx context.Context) (events <-chan WSMessage, errs <-chan error, done chan<- struct{}, err error)
```
